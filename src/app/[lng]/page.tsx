...
import { localesHomepage, PAGE_NAME } from "./locales"
import {
  localesCtaSection,
  CTA_SECTION_NAME,
...
export default async function HomePage({ params: { lng } }: BaseRouteProps) {
  const tHomepage = await useTranslation(lng, PAGE_NAME)
    .then((response: any) => translate(response.t, localesHomepage))
    .catch((err: any) => {
      console.error(err)
      return {}
    })
  ...
  const t = { ...tHomepage, ...tCtaSection }

  return <Homepage params={{ lng, t }} />
}
