...
import CustomerStories from "@/templates/CustomerStories"
import { localesCustomerStories, PAGE_NAME } from "./locales"
import {
  localesCtaSection,
  CTA_SECTION_NAME,
...
export default async function CustomerStoriesPage({
  ...
  const tCustomerStories = await useTranslation(lng, PAGE_NAME)
    .then((response: any) => translate(response.t, localesCustomerStories))
    .catch((err: any) => {
      console.error(err)
      return {}
    })
  ...
  const t = { ...tCustomerStories, ...tCtaSection }

  return <CustomerStories params={{ lng, t }} />
}
