...
import { localesAboutUs, PAGE_NAME } from "./locales"
import {
  localesCtaSection,
  CTA_SECTION_NAME,
...
export default async function AboutPage({ params: { lng } }: BaseRouteProps) {
  const tAboutUs = await useTranslation(lng, PAGE_NAME)
    .then((response: any) => translate(response.t, localesAboutUs))
    .catch((err: any) => {
      console.error(err)
      return {}
    })
  ...
  const t = { ...tAboutUs, ...tCtaSection }

  return <AboutUs params={{ lng, t }} />
}
