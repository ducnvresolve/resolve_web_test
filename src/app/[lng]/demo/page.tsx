...
import { localesDemo, PAGE_NAME } from "./locales"
...
export default async function DemoPage({ params: { lng } }: BaseRouteProps) {
  const t = await useTranslation(lng, PAGE_NAME)
    .then((response: any) => translate(response.t, localesDemo))
    .catch((err: any) => {
      console.error(err)
      return {}
    })

  return <Demo params={{ lng, t }} />
}
